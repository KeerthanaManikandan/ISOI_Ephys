function OIAshift2goodstim4(shiftfile3, percentage, shiftthreshold, NStim, NFrame, framerange)
...
%         mean4=mean((A(4, framerange, j, i)), 2);
        if shift1>shiftthreshold 
                C(1, j, i)=0;
        end
        if shift2>shiftthreshold 
                C(2, j, i)=0;
        end
...
fclose(fid);
fprintf('Goodstim4:\rBased on shifts(threshold=%1.2f), goodstim=%2.1f%%\r', shiftthreshold, sum(sum(C(1,:,:).*C(2,:,:)))*100/NBlock/NStim);
