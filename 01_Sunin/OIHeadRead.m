...
% input: 
%	oifile: filename include path info (e.g. 'c:\expt\data1.da')
%	system: either 'r' for redshirt system, or 'v' for vdaq system

% most usefule header info includes:
%	Frame width
%	Frame heigh
...
elseif (system=='r')	% for redshirt data
    ...
    anapar.NStim=fread(fid, 1, 'uint16=>double'); % in old data (<050621) these bytes were not used (will return NStim=0), need manual set NStim below
    %anapar.NStim =5;		% Modify this line if stim number is not available from data header
	if anapar.NStim<1	
	    fprintf('Note, old data (<050621) need manual enter stim number) \n');
	    fprintf('Please modify "OIReadHeader.m" at line 45 (delete % and enter correct condition number) before processing.\n');
	    return;
	end
...
elseif (system=='v') % for VDAQ data
    ...
    anapar.Cond = str2num(acListOfStimuli);
else
