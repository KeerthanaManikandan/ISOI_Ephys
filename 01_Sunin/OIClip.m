...
%           1: clipping to +-SD (value) (on each side of the median)
...
%  070420: corrected a bug in method3, not important
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
case 1    % 1: clipping to +-SD (value) 
    temp=reshape(imin, numel(imin), 1);    %transform image to 1-D array for calculation of median and stdev
...
case 2    %2: clipping to +-SD (value) with mask (immask)
    maskpixelnum=sum(immask(:));
    combined=[reshape(imin, numel(imin), 1), reshape(double(immask), numel(imin), 1)];    %following 3 lines for pick up useful pixels
...
case 3  %3: clipping to +-SD using the window specified in value (in this case it's a 2x3 matrix: x1, y1; x2, y2; and value(3, 1)=sd)
    immasked=imin(value(1,2):value(2,2), value(1,1):value(1,2));    %x1:x2, y1:y2
    frameMedian=median(immasked(:));    
    frameStd=std(immasked(:));
...
imresult=frame + (lowClip*(~Templ));%clip all avules lower than lowClip
