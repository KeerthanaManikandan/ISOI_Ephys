...
for iDate = 1:size(allDates,1)
    ...
    for iRun = 1:size(allRuns{iDate,1},1) % Get 
        ...
        nwbfile.general_subject = types.core.Subject(...
            ...
            'sex','M', ...
            'age',ageVal);
        % For the species go to this website and search for your species:
        % http://purl.obolibrary.org/obo/NCBITaxon_27679
        ...
        % Store ISOI file
        % ISOI device
        isoi_device = types.core.Device(... % Device type
            'description',  'ISOI camera',...
            'manufacturer', 'Photon Focus');
        nwbfile.general_devices.set('isoi_camera', isoi_device);

        % optical channel
        optical_channel = types.core.OpticalChannel(...
            'description',      'reflected light',...
            'emission_lambda',  double(recording.wavelength));

        % imaging plane — metadata only, triggers DANDI approach detection
        imaging_plane = types.core.ImagingPlane(...
            'description',        'cortical surface ISOI',...
            'device',             types.untyped.SoftLink(isoi_device),...
            'excitation_lambda',  double(recording.wavelength),...
            'imaging_rate',       double(recording.isoi_fs),...
            'indicator',          'reflectance',...
            'location',           recording.brain_region,...
        ...
        isoi_series = types.core.ImageSeries(...
            ...
            'data_unit' , 'Percent change in reflectance delR/R',...
            'format',       'processed');
        
        nwbfile.acquisition.set('ISOI', isoi_series);

        % Add the greens after spatially downsampling
        ref_image = types.core.GrayscaleImage(...
            'data',imresize(greenIm{iDate,iRun},1/spatialBin),...
            'description','reference blood vessel map for the recording');

        images = types.core.Images(...
            'description',  'reference image',...
            'GrayscaleImage', ref_image);


        nwbfile.acquisition.set('reference_image', images);
